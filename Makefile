...
DOCKER_ORG := blumeglobal
VERSION=$(shell docker run --rm -v${PWD}:/git -eGITHUB_AUTH_TOKEN=${GITHUB_AUTH_TOKEN} blumeglobal/jx-release-version --gh-owner=${ORG} --gh-repository=${NAME})
